...
def sub(i1,key1,key2,key3,key4,key5):
    ...
    S = Store(S, BitVecVal(10, 10), 150)

    S = Store(S, BitVecVal(6, 10), 132)
    S = Store(S, BitVecVal(14, 10),53)

    S = Store(S, BitVecVal(3, 10), I[3][s9_15b]) #key3=15
    S = Store(S, BitVecVal(15, 10), 236)
    S = Store(S, BitVecVal(11, 10), 68) 
    S = Store(S, BitVecVal(7, 10), 175)

    S = Store(S, BitVecVal(0, 10), I[s9_0][s9_0b])
    S = Store(S, BitVecVal(4, 10), 249)
    S = Store(S, BitVecVal(8, 10), 217) 
    S = Store(S, BitVecVal(12, 10), 165)
    ...
    ret = Store(ret, 0, ret[0] ^ (S[2 + 0] ^ S[3 + 0] ^ W[0][j + nb * n]))

    ret = Store(ret, 1 , S[1 + 0] << 1)
    ret = Store(ret, 1 , If(ret[1 + 0] >> 8 == 1, ret[1 + 0] ^ 283, ret[1 + 0]))
    x = S[2]
    x = x ^ (x << 1)
    ret = Store(ret, 1, If( x >> 8 == 1, ret[1 + 0] ^ (x ^ 283), ret[1 + 0] ^ x))
    ret = Store(ret, 1, ret[1 + 0] ^ (S[3 + 0] ^ S[0] ^ W[1][j + nb * n]))

    ret = Store(ret, 2, S[2 + 0] << 1)
    ret = Store(ret, 2, If(ret[2 + 0] >> 8 == 1, ret[2 + 0] ^ 283, ret[2 + 0]))
    x = S[3]
    x = x ^ (x << 1)
    ret = Store(ret, 2, If( x >> 8 == 1, ret[2 + 0] ^ (x ^ 283), ret[2 + 0] ^ x)) #key4 283
    ret = Store(ret, 2, ret[2 + 0] ^ (S[0] ^ S[1 + 0] ^ W[2][j + nb * n]))

    ret = Store(ret, key5, S[3 + 0] << 1) #key5 3
    ret = Store(ret, 3, If(ret[3 + 0] >> 8 == 1, ret[3 + 0] ^ 283, ret[3 + 0]))
    x = S[0]
    x = x ^ (x << 1)
    ret = Store(ret, 3, If( x >> 8 == 1, ret[3 + 0] ^ (x ^ 283), ret[3 + 0] ^ x))
    ret = Store(ret, 3, ret[3 + 0] ^ (S[1 + 0] ^ S[2 + 0] ^ W[3][j + nb * n]))
...
# s.add(simplify(sub(50,45,5,i3,23,90,123,6,20,69,12,54,89,45,78,90,key1_1,key2_1,key3_1,key4_1,key5_1) == out1))
# s.add(simplify(sub(50,45,5,i3,23,90,123,6,20,69,12,54,89,45,78,90,key1_2,key2_2,key3_2,key4_2,key5_2) == out2))
# dip2 = [23,10,13,65,0,11,10,78,65,23,11,10,8,0,11]

s.add(simplify(sub(i1,i2,i3,i4,i5,i6,i7,i8,i9,i10,i11,i12,i13,i14,i15,i16,key1_1,key2_1,key3_1,key4_1,key5_1))==out1)
s.add(simplify(sub(i1,i2,i3,i4,i5,i6,i7,i8,i9,i10,i11,i12,i13,i14,i15,i16,key1_2,key2_2,key3_2,key4_2,key5_2))==out2)
