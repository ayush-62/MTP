...
int main(){
    printf("W = Array('W', BitVecSort(10), ArraySort(BitVecSort(10), BitVecSort(10)))\n");
    for(int i = 0 ; i < 4 ; i++){
        printf("tm%d = Array('tm%d', BitVecSort(10), BitVecSort(10))\n",i+16,i);
        for(int j = 0 ; j < 120 ; j++){
            printf("tm%d = Store(tm%d, BitVecVal(%d, 10), BitVecVal(%d, 10))\n",i+16,i+16,j,word[i][j]);
        }
        printf("W = Store(W, BitVecVal(%d, 10), tm%d)\n",i,i+16);
    }
}