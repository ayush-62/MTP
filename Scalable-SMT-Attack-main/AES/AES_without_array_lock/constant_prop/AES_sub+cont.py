...
#-----------------------------------MixColumn AddRoundKey-----------------------------------------------------
    ...
    ret = Store(ret, 5, If(ret[5] >> 8 == key5, ret[5] ^ 283, ret[5]))
    x = S[6]
    x = x ^ (x << 1)
    ...
    S = Store(S, BitVecVal(4,10), 162)
    S = Store(S, BitVecVal(5,10), ret[5])
    S = Store(S, BitVecVal(6,10), 59)
    S = Store(S, BitVecVal(7,10), 39)
    S = Store(S, BitVecVal(8,10), 31)
    S = Store(S, BitVecVal(9,10), 1)
    S = Store(S, BitVecVal(10,10), 15)
    S = Store(S, BitVecVal(11,10), 4)
    S = Store(S, BitVecVal(12,10), 87)
    S = Store(S, BitVecVal(13,10), 144)
    S = Store(S, BitVecVal(14,10), 162)
    S = Store(S, BitVecVal(15,10), 211)
...
#----------------------------------Iteration 2-----------------------------------------------------------------
    ...
    S = Store(S, BitVecVal(5, 10),124)
    S = Store(S, BitVecVal(9, 10),96)
    S = Store(S, BitVecVal(13, 10),temp)

    temp = I[s2_2][s2_2b]
    S = Store(S, BitVecVal(2, 10), 118) #key2=10
    S = Store(S, BitVecVal(10, 10), temp)
    temp = I[s2_6][s2_6b]
    S = Store(S, BitVecVal(6, 10), 58)
    S = Store(S, BitVecVal(14, 10),226)

    temp = I[s2_3][s2_3b]
    S = Store(S, BitVecVal(3, 10), 102) #key3=15
    S = Store(S, BitVecVal(15, 10), 224)
    S = Store(S, BitVecVal(11, 10), 204)
    S = Store(S, BitVecVal(7, 10), temp)

    S = Store(S, BitVecVal(0, 10), I[s2_0][s2_0b])
    S = Store(S, BitVecVal(4, 10), 58)
    S = Store(S, BitVecVal(8, 10), 192)
    S = Store(S, BitVecVal(12, 10),91)
...
# #-----------------------------------MixColumn AddRoundKey-----------------------------------------------------
    ...
    x = x ^ (x << 1)
    ret = Store(ret, 3, If( x >> 8 == 1, ret[3] ^ (x ^ 283), ret[3] ^ x))
    ret = Store(ret, 3, ret[3] ^ (S[1] ^ 118 ^ W[3][4*n]))
...
#-----------------------------------MixColumn AddRoundKey-----------------------------------------------------
    ...
    n = BitVecVal(8,10)
    ret = If(key1 == 1,Store(ret, 0, S[0] << 1),Store(ret, 0, S[2] << 1))
    ret = Store(ret, 0, If(ret[0] >> 8 == 1, ret[0] ^ key5, ret[0]))
    x = S[1]
    x = x ^ (x << 1)
    ret = Store(ret, 0, If( x >> 8 == 1, ret[0] ^ (x ^ 283), ret[0] ^ x))
    ret = Store(ret, 0, ret[0] ^ (S[2] ^ S[3] ^ W[0][4*n]))
...
def sub(in1,key1,key2,key3,key4,key5,S,W,I,ret):
    # S = Array('S', BitVecSort(10), BitVecSort(10))
    # S2 = Array('S2', BitVecSort(10), BitVecSort(10))
    # I = Array('A', BitVecSort(10), ArraySort(BitVecSort(10), BitVecSort(10)))
    # W = Array('W', BitVecSort(10), ArraySort(BitVecSort(10), BitVecSort(10)))
    ...
    # ret = Store(ret, BitVecVal(0, 10), I[s9_0][s9_0b])
...
s.add(key5_1>=0,key5_1<=355)
s.add(key5_2>=0,key5_2<=355)
...
findOutput1(key1_1,key2_1,key3_1,key4_1,key5_1,key6_1,key7_1,key8_1,key9_1,key10_1,S,W,I,ret)
# print(simplify(findOutput1(key5_1,key6_1,key7_1,key8_1,key9_1,key10_1,S,W,I,ret)))
# print((simplify(findOutput1(5,10,15,283,3,5,4,7,8,10,S,W,I,ret))))
# exit()



# dip1 [215 0 0 0 1 83 0 0 0 0 10 0 0 0 0 52]
# dip2 = [inp1,23,32,13,65,0,11,32,78,65,23,11,32,8,0,11]
# print(simplify(sub(0,5,10,15,283,3)))
# exit()

oa = tuple.tuple1(BitVecVal(48,10),BitVecVal(155,10),BitVecVal(87,10),BitVecVal(48,10),BitVecVal(68,10),BitVecVal(48,10),BitVecVal(204,10),BitVecVal(82,10),BitVecVal(4,10),BitVecVal(69,10),BitVecVal(214,10),BitVecVal(65,10),BitVecVal(131,10),BitVecVal(41,10),BitVecVal(153,10),BitVecVal(204,10))
...
# s.add(simplify(findOutput1(key1_1,key2_1,key3_1,key4_1,key5_1,key6_1,key7_1,key8_1,key9_1,key10_1,S,W,I,ret))==out1)
# s.add(simplify(findOutput1(key1_2,key2_2,key3_2,key4_2,key5_2,key6_2,key7_2,key8_2,key9_2,key10_2,S,W,I,ret))==out2)

s.add(findOutput1(key1_1,key2_1,key3_1,key4_1,key5_1,key6_1,key7_1,key8_1,key9_1,key10_1,S,W,I,ret) == oa)
...
j = 0
start1 = time.time()
if(s.check(out3!=out4,   Or(key1_1 != key1_2, key2_1 != key2_2, key3_1 != key3_2, key4_1 != key4_2, key5_1 != key5_2)) == sat):
    m = s.model()
    #print(m)
    #print(m)
    print(str(m[i1]))
