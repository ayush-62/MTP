...
k1_1,k2_1,k3_1,k4_1,k5_1 = BitVecs('k1_1 k2_1 k3_1 k4_1 k5_1',32)
k1_2,k2_2,k3_2,k4_2,k5_2 = BitVecs('k1_2 k2_2 k3_2 k4_2 k5_2',32)
...
tuple.declare('tuple1',('1' , BitVecSort(32)),('2' , BitVecSort(32)),('3' , BitVecSort(32)),('4' , BitVecSort(32)),('5' , BitVecSort(32)),('6' , BitVecSort(32)),('7' , BitVecSort(32)),('8' , BitVecSort(32)),('9' , BitVecSort(32)),('10' , BitVecSort(32)),('11' , BitVecSort(32)),('12' , BitVecSort(32)),('13' , BitVecSort(32)),('14' , BitVecSort(32)),('15' , BitVecSort(32)),('16' , BitVecSort(32)))#,('17' , BitVecSort(32)),('18' , BitVecSort(32)),('19' , BitVecSort(32)),('20' , BitVecSort(32)),('21' , BitVecSort(32)),('22' , BitVecSort(32)),('23' , BitVecSort(32)),('24' , BitVecSort(32)),('25' , BitVecSort(32)),('26' , BitVecSort(32)),('27' , BitVecSort(32)),('28' , BitVecSort(32)),('29' , BitVecSort(32)),('30' , BitVecSort(32)),('31' , BitVecSort(32)),('32' , BitVecSort(32)))
...
for i in range(16):
    key = BitVecVal(i, 32)
    value = BitVecVal(statemt[i], 32)
    statemt_map.update({key:value})

for i in range(16):
    for j in range(16):
        key = (BitVecVal(i,32),(BitVecVal(j,32)))
        value = BitVecVal(Sbox[i][j],32)
        Sbox_map.update({key:value})

for i in range(4):
    for j in range(120):
        key = (BitVecVal(i,32),(BitVecVal(j,32)))
        value = BitVecVal(word[i][j],32)
        word_map.update({key:value})

# print(word_map[(BitVecVal(0,32),BitVecVal(0,32))])

def findOutput(key1,key2,key3,key4,nb,n):
    i = simplify(statemt_map[BitVecVal(1,32)]>> 4)
    j = simplify(statemt_map[BitVecVal(1,32)] & 0xf)
    temp = Sbox_map[(i,j)]
    
    i = simplify(statemt_map[BitVecVal(5,32)] >> 4) 
    j = simplify(statemt_map[BitVecVal(5,32)] & 0xf)
    statemt_map.update({BitVecVal(1,32):Sbox_map[(i,j)]})

    i = simplify(statemt_map[BitVecVal(9,32)] >> 4)
    j = simplify(statemt_map[key1] & 0xf) #key_1 = 9
    statemt_map.update({BitVecVal(5,32):Sbox_map[(i,j)]})

    i = simplify(statemt_map[BitVecVal(13,32)] >> 4) 
    j = simplify(statemt_map[BitVecVal(13,32)] & 0xf)
    statemt_map.update({BitVecVal(9,32):Sbox_map[(i,j)]})

    statemt_map.update({BitVecVal(13,32):temp})
    # print(statemt_map[BitVecVal(1,32)],statemt_map[BitVecVal(5,32)],statemt_map[BitVecVal(9,32)],statemt_map[BitVecVal(13,32)])

    i = simplify(statemt_map[BitVecVal(2,32)]>> 4)
    j = simplify(statemt_map[BitVecVal(2,32)] & 0xf)
    temp = Sbox_map[(i,j)]

    i = simplify(statemt_map[key2] >> 4) #key_2 = 10
    j = simplify(statemt_map[BitVecVal(10,32)] & 0xf)
    statemt_map.update({BitVecVal(2,32):Sbox_map[(i,j)]})

    statemt_map.update({BitVecVal(10,32):temp})

    i = simplify(statemt_map[BitVecVal(6,32)]>> 4)
    j = simplify(statemt_map[BitVecVal(6,32)] & 0xf)
    temp = Sbox_map[(i,j)]

    i = simplify(statemt_map[BitVecVal(14,32)] >> 4)
    j = simplify(statemt_map[key3] & 0xf) #key3 = 14
    statemt_map.update({BitVecVal(6,32):Sbox_map[(i,j)]})

    statemt_map.update({BitVecVal(14,32):temp})

    i = simplify(statemt_map[BitVecVal(3,32)]>> 4)
    j = simplify(statemt_map[BitVecVal(3,32)] & 0xf)
    temp = Sbox_map[(i,j)]

    i = simplify(statemt_map[key4] >> 4) #key4 = 15
    j = simplify(statemt_map[BitVecVal(15,32)] & 0xf)
    statemt_map.update({BitVecVal(3,32):Sbox_map[(i,j)]})

    i = simplify(statemt_map[BitVecVal(11,32)] >> 4)
    j = simplify(statemt_map[BitVecVal(11,32)] & 0xf)
    statemt_map.update({BitVecVal(15,32):Sbox_map[(i,j)]})

    i = simplify(statemt_map[BitVecVal(7,32)] >> 4)
    j = simplify(statemt_map[BitVecVal(7,32)] & 0xf)
    statemt_map.update({BitVecVal(11,32):Sbox_map[(i,j)]})

    i = simplify(statemt_map[BitVecVal(0,32)] >> 4)
    j = simplify(statemt_map[BitVecVal(0,32)] & 0xf)
    statemt_map.update({BitVecVal(0,32):Sbox_map[(i,j)]})

    i = simplify(statemt_map[BitVecVal(4,32)] >> 4)
    j = simplify(statemt_map[BitVecVal(4,32)] & 0xf)
    statemt_map.update({BitVecVal(4,32):Sbox_map[(i,j)]})

    i = simplify(statemt_map[BitVecVal(8,32)] >> 4)
    j = simplify(statemt_map[BitVecVal(8,32)] & 0xf)
    statemt_map.update({BitVecVal(8,32):Sbox_map[(i,j)]})

    i = simplify(statemt_map[BitVecVal(12,32)] >> 4)
    j = simplify(statemt_map[BitVecVal(12,32)] & 0xf)
    statemt_map.update({BitVecVal(12,32):Sbox_map[(i,j)]})

    statemt_map.update({BitVecVal(7,32):temp})

    o0 = statemt_map[BitVecVal(0,32)] + n
    o1 = statemt_map[BitVecVal(1,32)] + n
    o2 = statemt_map[BitVecVal(2,32)] + n
    o3 = statemt_map[BitVecVal(3,32)] + n
    o4 = statemt_map[BitVecVal(4,32)] + n
    o5 = statemt_map[BitVecVal(5,32)] + n 
    o6 = statemt_map[BitVecVal(6,32)] + n
    o7 = statemt_map[BitVecVal(7,32)] + n
    o8 = statemt_map[BitVecVal(8,32)] + n
    o9 = statemt_map[BitVecVal(9,32)] + n
    o10 = statemt_map[BitVecVal(10,32)] + n
    o11 = statemt_map[BitVecVal(11,32)] + n
    o12 = statemt_map[BitVecVal(12,32)] + n
    o13 = statemt_map[BitVecVal(13,32)] + n
    o14 = statemt_map[BitVecVal(14,32)] + n
    o15 = statemt_map[BitVecVal(15,32)] + n
...
print(gg.check(simplify(findOutput(BitVecVal(9,32),BitVecVal(10,32),BitVecVal(14,32),BitVecVal(15,32),BitVecVal(4,32),BitVecVal(5,32)))==oa))
...
while s.check(out1 != out2, Or(key1_1!=key1_2,key2_1!=key2_2,key3_1!=key3_2,key4_1!=key4_2)) == sat:
    m = s.model()
    #print(m)
    #print(str(m[k1_1])+" "+str(m[k2_1])+" "+str(m[k3_1])+" "+str(m[k4_1])+" "+str(m[k5_1])+" "+str(m[key1_1])+" "+str(m[key2_1]))
    #print(str(m[k1_2])+" "+str(m[k2_2])+" "+str(m[k3_2])+" "+str(m[k4_2])+" "+str(m[k5_2])+" "+str(m[key1_2])+" "+str(m[key2_2]))
    ia = str(m[nb]) + " " + str(m[n])
    [oa1,oa2,oa3,oa4,oa5,oa6,oa7,oa8,oa9,oa10,oa11,oa12,oa13,oa14,oa15,oa16]= Cexec(ia)
    #,BitVecVal(oa11,32),BitVecVal(oa12,32),BitVecVal(oa13,32),BitVecVal(oa14,32),BitVecVal(oa15,32),BitVecVal(oa16,32),BitVecVal(oa17,32),BitVecVal(oa18,32),BitVecVal(oa19,32),BitVecVal(oa20,32),BitVecVal(oa21,32),BitVecVal(oa22,32),BitVecVal(oa23,32),BitVecVal(oa24,32),BitVecVal(oa25,32),BitVecVal(oa26,32),BitVecVal(oa27,32),BitVecVal(oa28,32),BitVecVal(oa29,32),BitVecVal(oa30,32),BitVecVal(oa31,32),BitVecVal(oa32,32)
    oa = tuple.tuple1(BitVecVal(oa1,32),BitVecVal(oa2,32),BitVecVal(oa3,32),BitVecVal(oa4,32),BitVecVal(oa5,32),BitVecVal(oa6,32),BitVecVal(oa7,32),BitVecVal(oa8,32),BitVecVal(oa9,32),BitVecVal(oa10,32),BitVecVal(oa11,32),BitVecVal(oa12,32),BitVecVal(oa13,32),BitVecVal(oa14,32),BitVecVal(oa15,32),BitVecVal(oa16,32))
