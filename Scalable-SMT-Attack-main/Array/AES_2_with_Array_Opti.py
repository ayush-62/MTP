...
def findOutput(key1,key2,key3,key4,nb,n):
   ...
   # W = Array('W', BitVecSort(32), ArraySort(BitVecSort(32), BitVecSort(32)))
    ...
    S = Store(S, BitVecVal(1, 32), If(nb==4,  I[s5][s5b], S[1]))
    S = Store(S, BitVecVal(5, 32), If(nb==4,  I[s9][s9b], S[5])) #key1=9
    ...
    S = Store(S, BitVecVal(6, 32), If(nb==4,  I[s14][s14b], S[6])) #key3=14
    S = Store(S, BitVecVal(14, 32), If(nb==4, temp, S[14]))

    temp = If(nb==4,  I[s3][s3b], temp)
    S = Store(S, BitVecVal(3, 32), If(nb==4,  I[s15][s15b], S[3])) #key4=15
...
while s.check(key1_1==key1_2,key2_1==key2_2,key3_1==key3_2,key4_1==key4_2) != unsat:
    try:
        ...
        break
    pos_set.add(m)
    print("key="+"  "+str(m[key1_1])+" "+str(m[key2_1])+" "+str(m[key3_1])+" "+str(m[key4_1]))
...
g.add(simplify(findOutput(k1_1,k2_1,k3_1,k4_1,k5_1,key1_1,key2_1,nb,n)) == out1)
g.add(simplify(findOutput(k1_2,k2_2,k3_2,k4_2,k5_2,key1_2,key2_2,nb,n)) == out2)
g.add(nb==4)
g.add(n>=0 ,n<15)
...
while len(pos_l) > 1:
    m1 = pos_l[0]
    m2 = pos_l[1]
    print(str(m1[k1_2])+" "+str(m1[k2_2])+" "+str(m1[k3_2])+" "+str(m1[k4_2])+" "+str(m1[k5_2])+" "+str(m1[key1_2]))
    print(str(m2[k1_2])+" "+str(m2[k2_2])+" "+str(m2[k3_2])+" "+str(m2[k4_2])+" "+str(m2[k5_2])+" "+str(m2[key1_2]))
    g.push()
    g.add(k1_1==m1[k1_2],k2_1==m1[k2_2],k3_1==m1[k3_2],k4_1==m1[k4_2],k5_1==m1[k5_2],key1_1==m1[key1_2])
    g.add(k1_2==m2[k1_2],k2_2==m2[k2_2],k3_2==m2[k3_2],k4_2==m2[k4_2],k5_2==m2[k5_2],key1_2==m2[key1_2])
    if g.check(out1 != out2) == sat:
        ...
        if g.check(simplify(findOutput(m1[k1_2],m1[k2_2],m1[k3_2],m1[k4_2],m1[k5_2],m1[key1_2],m[nb],m[n])) == oa) == unsat:
            pos_l.remove(m1)
        if g.check(simplify(findOutput(m2[k1_2],m2[k2_2],m2[k3_2],m2[k4_2],m2[k5_2],m2[key1_2],m[nb],m[n])) == oa) == unsat:
            pos_l.remove(m2)
    else:
        pos_l.remove(m1)
    g.pop()
