...
def findOutput(k1,k2,k3,k4,k5,key1,key2,key3,nb,n):
    S = Array('S', BitVecSort(32), BitVecSort(32))
    I = Array('A', BitVecSort(32), ArraySort(BitVecSort(32), BitVecSort(32)))
    ...
    temp = If(Xor(nb==4,k1),  I[S[2] >> 4][S[2] & 0xf], temp) #k1 False
    S = Store(S, BitVecVal(2, 32), If(nb==4,  I[S[10] >> 4][S[10] & 0xf], S[2]))
    S = Store(S, BitVecVal(10, 32), If(nb==4, temp, S[10]))
    temp = If(And(nb==4,k2),  I[S[6] >> 4][S[6] & 0xf], temp) #k2 True
    ...
    S = Store(S, BitVecVal(11, 32), If(Xor(nb==4,k3),  I[S[7] >> 4][S[7] & 0xf], S[11])) # k3 False
    ...
    S = Store(S, BitVecVal(8, 32), If(Xor(nb!=4,k4),  I[S[8] >> 4][S[8] & 0xf], S[8])) # k4 True
    ...
    ret = Store(ret, key2 + j * 4, If(j < nb, S[1 + j * 4] << 1, ret[1 + j * 4])) #key2 1
    ret = Store(ret, 1 + j * 4, If(And(j < nb, ret[1 + j * 4] >> 8 == 1), ret[1 + j * 4] ^ 283, ret[1 + j * 4]))
    x = S[2 + j * 4]
    x = x ^ (x << 1)
    ret = Store(ret, 1 + j * key3, If(And(j < nb, x >> 8 == 1), ret[1 + j * 4] ^ (x ^ 283), ret[1 + j * 4])) #key3 4
    ret = Store(ret, 1 + j * 4, If(And(j < nb, Not(x >> 8 == 1)), ret[1 + j * 4] ^ x, ret[1 + j * 4]))
    ret = Store(ret, 1 + j * 4, If(j < nb, ret[1 + j * 4] ^ (S[3 + j * 4] ^ S[j * 4] ^ W[1][j + nb * n]), ret[1 + j * 4]))
    ...
    o8 = ret[8] 
    o9 = ret[9]
    o10 = If(k5,ret[10],ret[0]) #k5 True
    o11 = ret[11]
    o12 = ret[12]
    o13 = ret[13]
    o14 = ret[14]
...
s.add(simplify(sub(key1_1,k1_1,k2_1,k3_1,k4_1,nb,n)) == out3)
s.add(simplify(sub(key1_2,k1_2,k2_2,k3_2,k4_2,nb,n)) == out4)
s.add(nb == 4)#nb == 4, nb == 6, 
s.add(n >= 0, n < 15)
s.add(key1_1>=0)
s.add(key1_2>=0)
s.add(key2_1>=0)
...
while s.check(out3 != out4, Or(k1_1 != k1_2,k2_1 != k2_2 , k3_1 != k3_2 ,k4_1 != k4_2,k5_1 != k5_2,key1_1 != key1_2,key2_1 != key2_2,key3_1 != key3_2)) == sat:
    m = s.model()
    print(m)
    ia = str(m[nb]) + " " + str(m[n])
    ...
    s.add(simplify(findOutput(k1_1,k2_1,k3_1,k4_1,k5_1,key1_1,key2_1,key3_1,m[nb],m[n])) == oa)
    s.add(simplify(findOutput(k1_2,k2_2,k3_2,k4_2,k5_2,key1_2,key2_2,key3_2,m[nb],m[n])) == oa)
    j = j + 1

p=0
while s.check(k1_1 == k1_2,k2_1 == k2_2,k3_1 == k3_2,k4_1 == k4_2,k5_1 == k5_2,key1_1 == key1_2,key2_1 == key2_2,key3_1 == key3_2) != unsat:
    try:
        m = s.model()
    except:
        break_away = True
        break
    pos_set.add(m)
    print(str(m[key1_1])+" "+str(m[key2_1])+" "+str(m[key3_1])+" "+str(m[k1_1])+" "+str(m[k2_1])+" "+str(m[k3_1])+" "+str(m[k4_1])+" "+str(m[k5_1]))

    if len(pos_set) > rem_key_max:
        break
    s.add(Or(k1_1 != m[k1_1] , k2_1 != m[k2_1] , k3_1 != m[k3_1] , k4_1 != m[k4_1] , k5_1 != m[k5_1] , key1_1 != m[key1_1] , key2_1 != m[key2_1] , key3_1 != m[key3_1]))
    s.add(Or(k1_2 != m[k1_2] , k2_2 != m[k2_2] , k3_2 != m[k3_2] , k4_2 != m[k4_2] , k5_2 != m[k5_2] , key1_2 != m[key1_2] , key2_2 != m[key2_2] , key3_2 != m[key3_2]))
    p = p + 1
...
g.add(simplify(findOutput(k1_1,k2_1,k3_1,k4_1,k5_1,key1_1,key2_1,key3_1,nb,n)) == out1)
g.add(simplify(findOutput(k1_2,k2_2,k3_2,k4_2,k5_2,key1_2,key2_2,key3_2,nb,n)) == out2)
g.add(nb==4)
g.add(n>=0 ,n<15)
...
while len(pos_l) > 1:
    ...
    m2 = pos_l[1]
    g.push()
    g.add(k1_1 == m1[k1_2],k2_1 == m1[k2_2] , k3_1 == m1[k3_2], k4_1 == m1[k4_2], k5_1 == m1[k5_2],key1_1 == m1[key1_2],key2_1 == m1[key2_2],key3_1 == m1[key3_2])
    g.add(k1_2 == m2[k1_2],k2_2 == m2[k2_2],k3_2 == m2[k3_2],k4_2 == m2[k4_2],k5_2 == m2[k5_2],key1_2 == m2[key1_2],key2_2 == m2[key2_2],key3_2 == m2[key3_2])
    if g.check(out1 != out2) == sat:
        m = g.model()
        ia = str(m[nb]) + " " + str(m[n])
        [oa1,oa2,oa3,oa4,oa5,oa6,oa7,oa8,oa9,oa10,oa11,oa12,oa13,oa14,oa15,oa16] = Cexec(ia)
        oa = tuple.tuple1(BitVecVal(oa1,32),BitVecVal(oa2,32),BitVecVal(oa3,32),BitVecVal(oa4,32),BitVecVal(oa5,32),BitVecVal(oa6,32),BitVecVal(oa7,32),BitVecVal(oa8,32),BitVecVal(oa9,32),BitVecVal(oa10,32),BitVecVal(oa11,32),BitVecVal(oa12,32),BitVecVal(oa13,32),BitVecVal(oa14,32),BitVecVal(oa15,32),BitVecVal(oa16,32))
        if g.check(simplify(findOutput(m1[k1_2],m1[k2_2],m1[k3_2],m1[k4_2],m1[k5_2],m1[key1_2],m1[key2_2],m1[key3_2],m[nb],m[n])) == oa) == unsat:
            pos_l.remove(m1)
        if g.check(simplify(findOutput(m2[k1_2],m2[k2_2],m2[k3_2],m2[k4_2],m2[k5_2],m2[key1_2],m2[key2_2],m2[key3_2],m[nb],m[n])) == oa) == unsat:
            pos_l.remove(m2)
    else:
        pos_l.remove(m1)
    g.pop()
