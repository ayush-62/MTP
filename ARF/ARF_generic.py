...
#function definition 
def findOutput(i1,i2,i3,i4,i6,G1,G2,GG1,GG2,key1,key2,key3,key4,key5,k1,k2,k3,k4,k5,k6,k7,k8,k9,k10):

    op1 = BitVecVal(17,32) * (i1 + key1) #key1=23
    op2 = If(k1,GG2*i2,BitVecVal(0,32)) # k1=True
    op3 = G1 * i2
    op4 = If(k2,BitVecVal(0,32),G2 * i1) #k2=False
    op5 = If(Not(Xor(G1>10,k3)), G1*i3, BitVecVal(0,32)) # k3=True
    op5 = If(Not(Xor(G1>10,k3)), op5 + (GG1 * key2), op5) #key2=11
    op6 = If(Not(Xor(G1>10,k3)), op5 - op4, BitVecVal(0,32))
    op5 = If(Not(Xor(G1>10,k3)), op5, i3*i4)
    op6 = If(Not(Xor(G1>10,k3)), op6, op5 - (op3 * key3)) #key3=7
    op6 = If(Xor(op5<op4,k4), G2*i4, op6) #k4=False
    op6 = If(Xor(op5<op4,k4), op6 - (i3*key4), op6) #key4=17
    op17 = If(Xor(op5<op4,k4), op6 - G2, BitVecVal(0,32))
    op17 = If(Xor(op5<op4,k4), op17, op2 - op4)
    op2 = If(Xor(op5<op4,k4), op2, op4 - (op17 * key5)) #key5=13
    op17 = If(Xor(op5<op4,k4), op17, op17 - op2)
    op7 = If(k5,G1*i4,BitVecVal(0,32)) #k5=True
    op8 = G2*i3
    ...
    op14 = If(k9,i6+ op12,BitVecVal(0,32)) #k9=True
    o2 = op14
    op15 = G1* op14 
    op16 = op13 * G2
...
# print(gg.check(findOutput(1,2,3,5,7,11,13,17,19,23,11,7,17,13,True,False,True,False,True,False,True,False,True,False)==oa))
...
s.add(findOutput(i1,i2,i3,i4,i6,G1,G2,GG1,GG2,key1_1,key2_1,key3_1,key4_1,key5_1,k1_1,k2_1,k3_1,k4_1,k5_1,k6_1,k7_1,k8_1,k9_1,k10_1) == out1)
s.add(findOutput(i1,i2,i3,i4,i6,G1,G2,GG1,GG2,key1_2,key2_2,key3_2,key4_2,key5_2,k1_2,k2_2,k3_2,k4_2,k5_2,k6_2,k7_2,k8_2,k9_2,k10_2) == out2)
s.add(i1>=0,i1<=255)
s.add(i2>=0,i2<=255)
s.add(i3>=0,i3<=255)
s.add(i4>=0,i4<=255)
...
while s.check(out1 != out2, Or(key1_1 != key1_2,key2_1 != key2_2,key3_1 != key3_2,key4_1 != key4_2,key5_1 != key5_2,k1_1!=k1_2,k2_1!=k2_2,k3_1!=k3_2,k4_1!=k4_2,k5_1!=k5_2,k6_1!=k6_2,k7_1!=k7_2,k8_1!=k8_2,k9_1!=k9_2,k10_1!=k10_2)) == sat:
    ...
    ia = str(m[i1]) +" "+str(m[i2])+" "+str(m[i3])+" "+str(m[i4])+" "+str(m[i6])+" "+str(m[G1])+" "+str(m[G2])+" "+str(m[GG1])+" "+str(m[GG2])
    #using the input to get the correct output for the DIP from the oracle 
    [oa1,oa2,oa3,oa4] = Cexec(ia)
    oa = tuple.tuple(BitVecVal(oa1,32),BitVecVal(oa2,32),BitVecVal(oa3,32),BitVecVal(oa4,32))
    #constraints added with the DIP
    s.add(findOutput(m[i1],m[i2],m[i3],m[i4],m[i6],m[G1],m[G2],m[GG1],m[GG2],key1_1,key2_1,key3_1,key4_1,key5_1,k1_1,k2_1,k3_1,k4_1,k5_1,k6_1,k7_1,k8_1,k9_1,k10_1) == oa)
    s.add(findOutput(m[i1],m[i2],m[i3],m[i4],m[i6],m[G1],m[G2],m[GG1],m[GG2],key1_2,key2_2,key3_2,key4_2,key5_2,k1_2,k2_2,k3_2,k4_2,k5_2,k6_2,k7_2,k8_2,k9_2,k10_2) == oa)
    j = j + 1
