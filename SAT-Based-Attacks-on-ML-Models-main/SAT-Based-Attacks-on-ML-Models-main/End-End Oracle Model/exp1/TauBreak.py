...
while not prune_key_set:
    ...
    while s.check(output1[0] != output2[0], Or(l1n1v1[0] != l1n1v2[0], l1n2v1[0] != l1n2v2[0])) == sat:
    ...
    while s.check(l1n1v1[0] == l1n1v2[0], l1n2v1[0] == l1n2v2[0]) != unsat:
        try:
            m = s.model()
        except:
            break_away = True
        ...
        s.add(Or(l1n1v1[0] != m[l1n1v1[0]], l1n2v1[0] != m[l1n2v1[0]]))
        s.add(Or(l1n1v2[0] != m[l1n1v2[0]], l1n2v2[0] != m[l1n2v2[0]]))
        p = p + 1
...
g.add(simplify(neuralNetwork(input, l1n1v1, l1n2v1)[0] == output1[0]))
g.add(simplify(neuralNetwork(input, l1n1v2, l1n2v2)[0] == output2[0]))
...
while len(pos_l) > 1:
    ...
    m2 = pos_l[1]
    g.push()
    g.add(simplify(l1n1v1[0] != m1[l1n1v1[0]], l1n2v1[0] != m1[l1n2v1[0]]))
    g.add(simplify(l1n1v2[0] != m2[l1n1v2[0]], l1n2v2[0] != m2[l1n2v2[0]]))
    if g.check(output1[0] != output2[0]) == sat:
        ...
        if g.check(simplify(neuralNetwork(in1, l1n1m1, l1n2m1)[0] == out)) == unsat:
            pos_l.remove(m1)
        if g.check(simplify(neuralNetwork(in2, l1n1m2, l1n2m2)[0] == out)) == unsat:
            pos_l.remove(m2)
    else:
        pos_l.remove(m1)
    g.pop()
