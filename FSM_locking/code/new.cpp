...
unordered_map<string, vector<string>> fanout;
...
*/
void DFS(string start_node, string currNode , unordered_map<string,bool> &visitedNode)
{
    if(visitedNode[currNode])
        return;
    fanout[start_node].push_back(currNode);
    ...
    if(inputToOutputItr != inputToOutput.end())
    {
        for(i=0;i<inputToOutputItr->second.size();i++)
        {
            ...
            DFS(start_node, inputToOutputItr->second[i] , visitedNode);
        }
    }
}
void constructGraph()
    ...
    while(getline(fin,line))
    {
        ...
        for(int i=3;i<tokens.size()-1;i++)
        {
            ...
            if(operandType == 'Y' || operandType == 'Q' || operandType == 'X' || operandType == 'N')
            {
                ...
                // cout<<"output operand" << input<<endl;
            }
            else
            {
        ...
        for(i=0;i<input_operands.size();i++)
        {
            if(input_operands[i] == "rst" || op.substr(0,2) != "df" || input_operands[i] == "clock") 
                continue;
...
void circuit_graph(){
    ...
    for(auto it : outputToInput){
        string input_oprand  = " ", output_oprand = " ";
        if(operatorOperand.find(it.first) != operatorOperand.end())
            output_oprand = operatorOperand[it.first];
        if(output_oprand[0] == 'd'){
                cout<<"\""<<output_oprand<<" "<<outputToRegister[it.first]<<"\""<<"[shape = box,style=filled,color=\".7 .3 1.0\"]"<<endl;
            }
        else if(output_oprand[0] == 'm'){
                cout<<"\""<<output_oprand<<" "<<outputToRegister[it.first]<<"\""<<"[shape = box,style=filled,color=\".2 .4 1.0\"]"<<endl;
            }
        else{
            cout<<"\""<<output_oprand<<" "<<outputToRegister[it.first]<<"\""<<"[shape = box,style=filled,color=\".6 .5 1.0\"]"<<endl;
        }
        for(auto jt : it.second){
            cout<<"\""<<jt<<"\""<<"->"<<"\""<<output_oprand<<" "<<outputToRegister[it.first]<<"\""<<endl; 
        }
        cout<<"\""<<output_oprand<<" "<<outputToRegister[it.first]<<"\""<<"->"<<"\""<<it.first<<"\""<<endl;
    }
    cout<<endl<<"}";
}
void register_fanout(){
    for(auto it : registerList){
        for(auto jt : registerToOutput[it]){
            visited.clear();
            DFS(it, jt, visited);
        }
        // DFS(registerToOutput[it].first, registerToOutput[it].second, visited)
    }
}
void print(){
    cout<<" -----------------outputToInput----------------";
